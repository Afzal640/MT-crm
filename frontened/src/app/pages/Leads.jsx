...
const [assignData, setAssignData] = useState({
  ...
  useEffect(() => {
    const fetchProductionUsers = async () => {
      const token = localStorage.getItem("token");
      try {
        const res = await axios.get("https://mt-crm-46da.vercel.app/api/auth/production-users", {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingLead) {
        res = await axios.put(
          `https://mt-crm-46da.vercel.app/api/leads/${editingLead._id}`,
          formData,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        ...
        res = await axios.post(
          "https://mt-crm-46da.vercel.app/api/leads",
          formData,
          { headers: { Authorization: `Bearer ${token}` } }
        );
  ...
  const handleDeleteLead = async (id) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`https://mt-crm-46da.vercel.app/api/leads/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
...
const convertLeadToProject = async () => {
  try {
    ...
    await axios.post(
      "https://mt-crm-46da.vercel.app/api/projects/from-lead",
      {

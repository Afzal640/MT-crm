...
const uploadFile = async () => {
  try {
    ...
    await axios.post(
      `https://mt-crm-46da.vercel.app/api/projects/${id}/files`,
...
export const ProjectDetails = () => {
  ...
  const fetchProject = async () => {
    try {
      const token = localStorage.getItem("token");
      const res = await axios.get(`https://mt-crm-46da.vercel.app/api/projects/${id}`, {
  ...
  const updateStatus = async (newStatus) => {
    try {
      const token = localStorage.getItem("token");
      await axios.patch(
        `https://mt-crm-46da.vercel.app/api/projects/${id}/status`,
        { status: newStatus },
        { headers: { Authorization: `Bearer ${token}` } }
      );
